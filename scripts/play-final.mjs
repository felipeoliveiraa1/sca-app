...
await p.getByRole("button",{name:/convidado/i}).first().click();
await p.locator("input[placeholder='Seu nome']").fill("José");
await p.getByRole("button",{name:/Entrar no clube/i}).first().click();
await p.waitForTimeout(900);
console.log("na Play (Episódios):", await p.getByText("Episódios").first().isVisible().catch(()=>false));
await p.getByText("Em alta").first().click();   // episódio em destaque
await p.waitForTimeout(900);
